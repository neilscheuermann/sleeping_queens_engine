defmodule QueenCardTest do
  ...
  describe "queens_pile_shuffled/1" do
    test "it returns a total of 16 queen cards", %{queens_pile: queens_pile} do
      assert Enum.count(queens_pile) == 16
      assert [%QueenCard{} | _rest] = queens_pile
    end

    test "it returns the cards in a shuffled order", %{queens_pile: queens_pile} do
      new_queens_pile = QueenCard.queens_pile_shuffled()
      refute queens_pile == new_queens_pile
    end

    test "each queen has a value of 5, 10, 15, or 20", %{queens_pile: queens_pile} do
      assert Enum.all?(queens_pile, &(&1.value in @valid_queen_values))
    end

    test "four of the queen cards are special (rose, strawberry, cat, and dog)", %{
      queens_pile: queens_pile
    } do
